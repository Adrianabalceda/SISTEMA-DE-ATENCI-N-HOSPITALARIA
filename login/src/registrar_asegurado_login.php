<?php
    ...
    $contraseña = strtoupper($nombresAsegurado.' '.$apellidosAsegurado);

    $sql = "SELECT * FROM 'asegurado' WHERE DNI = '$dniAsegurado'";
    $result = mysqli_query($conexion, $sql);
    $count = mysqli_num_rows($result);
    
    $sql = "SELECT * FROM 'familiar' WHERE DNI = '$dniAsegurado'";
    $result = mysqli_query($conexion, $sql);
    ...
    if($count !== 1 && $count2 !== 1) {
        ...
        VALUES ('$dniAsegurado', '$usuario','$contraseña','$nombresAsegurado', '$apellidosAsegurado', '$emailAsegurado', '$celularAsegurado','$fechaAsegurado')";
